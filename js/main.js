...
alert("Bienvenido a DeMotores! Tenemos una amplia variedad de vehículos a la venta, si tienes alguna consulta no dudes en llamarnos!");
...
function mostrarModelosPorMarca() {
  const marcas = Object.keys(marcasDeAutos);
  let mensaje = "Por favor, elija una marca:\n";
  ...
  if (seleccion && !isNaN(seleccion)) {
    ...
    if (indice >= 0 && indice < marcas.length) {
      const marcaSeleccionada = marcas[indice];
      mostrarModelosDeMarca(marcaSeleccionada);
    } else {
      alert("Opción no válida. Por favor, seleccione una marca válida.");
    }
  } else {
    alert("Debe ingresar un número válido para seleccionar una marca.");
  }
}
